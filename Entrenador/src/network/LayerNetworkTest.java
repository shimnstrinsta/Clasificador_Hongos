...
public class LayerNetworkTest {
	...
	public void networkLogicGateSingleOutputTest(LinkedList<double[]> input, LinkedList<double[]> expectedOutput, String gateName, boolean verbose) throws Exception {
		...
		for (int i = 0; i < casosTotales; i++) {
			...
			if (seed < 0.25)
				...
				randomIndex = 3;
			testInput[i] = input.get(randomIndex);
			testOutput[i] = expectedOutput.get(randomIndex);
		}
		
		double threshold = 0.5;
		HashMap<String, Double> metricsData = utils.binaryMetrics(net, testInput, testOutput, threshold, verbose);
			
		assertTrue(metricsData.get("tasa_aciertos") > 0.70);
		assertTrue(metricsData.get("tasa_desaciertos") < 0.20);
	}	
	public void networkLogicGateDoubleOutputTest(LinkedList<double[]> input, LinkedList<double[]> expectedOutput, String gateName, boolean verbose) throws Exception {
		NeuralNetwork net = new NeuralNetwork(0.1);
		net.newInputLayer(2, 3);
		net.newHiddenLayer(4);
		...
		HashMap<String, Double> metricsData = utils.binaryDualOutputMetrics(net, testInput, testOutput, verbose);
