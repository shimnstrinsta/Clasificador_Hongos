...
public class utils {
    ...
    /**
     * Genera metricas para una red neuronal, a partir de datos de prueba; las
     * cuales son retornadas en un HashMap. Las metricas disponibles se pueden
     * obtener con las llaves: "tasa_aciertos", "tasa_desaciertos", "precision"
     * y "recall".
     *
     * Para predicciones binarias en las cuales hay solo 1 output y un threshold
     * desde el cual se interperta como clase 1.
     *
     ...
     */
    public static HashMap<String, Double> binaryMetrics(NeuralNetwork net, LinkedList<double[]> input, LinkedList<double[]> expectedOutput, double threshold, boolean verbose) throws Exception {
        ...
        int aciertos = 0, desaciertos = 0;
        
        for (int i = 0; i < input.size(); i++) {
            int[] realOutput = net.binaryPredict(input.get(i), threshold);
                        
            System.out.println("Esperado:" + expectedOutput.get(i)[0] + " Real:" + realOutput[0]);            
            
            if  (expectedOutput.get(i)[0] == realOutput[0]) {
                aciertos++;
            } else{
                desaciertos++;
            }                                  
        ...
        double tasaAciertos = (aciertos) * 1.0 / input.size();
        double tasaDesaciertos = (desaciertos) * 1.0 / input.size();

        metricsData.put("tasa_aciertos", tasaAciertos);
        metricsData.put("tasa_desaciertos", tasaDesaciertos);
        metricsData.put("precision", tasaAciertos * 100);
        ...
        if (verbose) {
            ...
            System.out.println("Numero de experimentos: " + input.size());
     ...
     * y "recall".
     *
     * Para predicciones binarias en las cuales hay 2 output, y el indice del
     * output predominante determina la clase (1 o 0)
     *
     * @param net Red que se desea evaluar.
     * @param input Datos de entrada de prueba.
     * @param expectedOutput Datos de salida esperados para los datos de prueba.
     ...
     * @throws Exception Si entrada y salida esperada tienen distintas salidas.
     */
    public static HashMap<String, Double> binaryDualOutputMetrics(NeuralNetwork net, LinkedList<double[]> input, LinkedList<double[]> expectedOutput, boolean verbose) throws Exception {
        ...
        for (int i = 0; i < input.size(); i++) {
            double[] realOutput = net.predict(input.get(i));
            // expected class is 0
            if (expectedOutput.get(i)[0] >= expectedOutput.get(i)[1] && realOutput[0] >= realOutput[1]) {
                verdaderosNegativos++;
            } else if (expectedOutput.get(i)[0] >= expectedOutput.get(i)[1] && realOutput[0] < realOutput[1]) {
                falsosPositivos++;
            } else if (expectedOutput.get(i)[0] < expectedOutput.get(i)[1] && realOutput[0] < realOutput[1]) {
                verdaderosPositivos++;
            } else if (expectedOutput.get(i)[0] < expectedOutput.get(i)[1] && realOutput[0] >= realOutput[1]) {
                ...
                System.out.println("El esperado es " + expectedOutput.get(i)[0] + ". El real es " + realOutput[0]);
            }
        }

        double tasaAciertos = (verdaderosPositivos + verdaderosNegativos) * 1.0 / input.size();
        double tasaDesaciertos = (falsosPositivos + falsosNegativos) * 1.0 / input.size();
    ...
    /**
     * Metodo para post-procesar los datos. Util en caso de desbalance de
     * clases, se puede hacer oversampling o subsampling. Ademas al final
     * distribuye los datos intercalados por clases.
     *
